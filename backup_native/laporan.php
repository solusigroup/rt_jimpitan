...
// Query Total
$total_res = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM jimpitan_harian");
if ($total_row = mysqli_fetch_assoc($total_res)) {
    $total_tugas = intval($total_row['total']);
}

// Query Selesai
$selesai_res = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM jimpitan_harian WHERE status = 'Sudah Dikerjakan'");
if ($selesai_row = mysqli_fetch_assoc($selesai_res)) {
    $selesai = intval($selesai_row['total']);
}

// Query Belum Selesai
$belum_res = mysqli_query($koneksi, "SELECT COUNT(*) as total FROM jimpitan_harian WHERE status = 'Belum Dikerjakan'");
if ($belum_row = mysqli_fetch_assoc($belum_res)) {
    $belum_selesai = intval($belum_row['total']);
}

// Hitung Kepatuhan
if ($total_tugas > 0) {
    $compliance_rate = round(($selesai / $total_tugas) * 100);
}
?>
...
<head>
    ...
    <title>Laporan Jimpitan - RT Digital</title>
...
<div class="container py-2 mb-5">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-6 col-md-3">
            <div class="card stat-card p-3 h-100">
                ...
                <h3 class="fw-bold text-dark mb-0"><?= $total_tugas ?> <span class="fs-6 fw-normal text-muted">Hari</span></h3>
            </div>
        </div>
        ...
        <div class="col-6 col-md-3">
            <div class="card stat-card p-3 h-100">
                ...
                <h3 class="fw-bold text-warning mb-0"><?= $belum_selesai ?> <span class="fs-6 fw-normal text-muted">Hari</span></h3>
            </div>
        </div>
        ...
        <div class="col-6 col-md-3">
            <div class="card stat-card p-3 h-100">
                ...
                <h3 class="fw-bold text-info mb-1"><?= $compliance_rate ?>%</h3>
                <div class="progress compliance-bar bg-light">
                    <div class="progress-bar bg-info" role="progressbar" style="width: <?= $compliance_rate ?>%" aria-valuenow="<?= $compliance_rate ?>" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
        <div class="card-header bg-white border-0 py-3 fw-bold text-dark fs-5 d-flex justify-content-between align-items-center">
            ...
            <span class="badge bg-secondary rounded-pill fw-semibold text-white fs-6">Total Laporan: <?= $total_tugas ?></span>
        </div>
        ...
        <div class="card-body p-0 border-top">
            <div id="logsContainer" class="d-flex flex-column">
                ...
                $query = "SELECT jh.id, jh.tanggal, w.nama, w.no_rumah, w.no_wa, jh.status, jh.waktu_update
                          FROM jimpitan_harian jh
                          JOIN warga w ON jh.warga_id = w.id
                          ORDER BY jh.tanggal DESC, jh.waktu_update DESC";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        $weton = getHariPasaran($tanggal_raw);
                        ...
                        $bln = $bulan_indo[intval(date('m', $timestamp))];
                        $thn = date('Y', $timestamp);
                        $formatted_date = "$hari_jawa $pasaran_jawa, $tgl $bln $thn";
                        ...
                        // Format updated time
                        $update_time = date('H:i', strtotime($row['waktu_update'])) . " WIB";
                        
                        // Status badge
                        $status_badge = "";
                        if ($status == 'Sudah Dikerjakan') {
                            $status_badge = "<span class='badge bg-success text-white px-3 py-1.5 fw-semibold rounded-pill d-inline-flex align-items-center gap-1.5'><i class='bi bi-check-all fs-6'></i> Sudah Selesai</span>";
                        } else {
                            $status_badge = "<span class='badge bg-warning text-dark px-3 py-1.5 fw-semibold rounded-pill d-inline-flex align-items-center gap-1.5'><i class='bi bi-clock-history fs-6'></i> Belum Selesai</span>";
                        }

                        echo "
                        <div class='log-card p-3 border-bottom bg-white d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3' data-search='{$nama} {$no_rumah}' data-status='{$status}'>
                            ...
                            <div class='d-flex align-items-center gap-3'>
                                ...
                                <div>
                                    <div class='fw-bold text-dark fs-6'>{$formatted_date}</div>
                                    <div class='text-secondary small d-flex align-items-center gap-1.5 mt-0.5 flex-wrap'>
                                        ...
                                        <span class='badge bg-light text-dark border px-2 py-0.5 small rounded-1'>No. {$no_rumah}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class='d-flex flex-row flex-md-column align-items-center align-items-md-end justify-content-between gap-2 border-top border-md-0 pt-2 pt-md-0'>
                                ...
                                <div class='text-secondary small' style='font-size: 0.78rem;'>
                                    <i class='bi bi-clock me-1'></i>Waktu lapor: {$update_time}
                                </div>
                            </div>
                        </div>";
                    }
